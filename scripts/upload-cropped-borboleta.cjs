/**
 * Faz upload de 3 novas fotos de Cropped Borboleta Rosé para o Supabase
...
const TMP = path.resolve(__dirname, "..", "tmp_borboleta");
...
const ITEMS = Array.from({ length: 3 }, (_, i) => ({
  index: i + 1,
  category: "cropped",
  name: "Cropped Borboleta Rosé",
}));

async function uploadOne(item) {
  const localPath = path.join(TMP, `c-${String(item.index).padStart(3, "0")}.png`);
  if (!fs.existsSync(localPath)) {
    throw new Error("Imagem nao encontrada: " + localPath);
  }
  const buf = fs.readFileSync(localPath);
  const ts = Date.now();
  const storagePath = `cropped/${ts}-cropped-borboleta-rose-${String(item.index).padStart(3, "0")}.png`;
...
(async () => {
  console.log(`Enviando ${ITEMS.length} novas fotos de Cropped Borboleta Rosé...\n`);
  let ok = 0;
  let err = 0;
