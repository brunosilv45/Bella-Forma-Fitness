...
$dst = 'C:\Users\Bruno Carvalho\Documents\BellaFormaFitness\tmp_short'
if (!(Test-Path -LiteralPath $dst)) { New-Item -ItemType Directory -Path $dst | Out-Null }
$files = Get-ChildItem -Path $src -Filter '*2026-05-12_at_11.19*.png' | Sort-Object Name
$i = 0
foreach ($f in $files) {
  $i++
  $idx = "{0:D3}" -f $i
  $longSrc = '\\?\' + $f.FullName
  $dstPath = Join-Path $dst "s-$idx.png"
