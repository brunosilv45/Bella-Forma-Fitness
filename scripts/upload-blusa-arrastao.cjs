/**
 * Faz upload de 6 novas fotos de Blusa Arrastão Azul para o Supabase Storage
 * e cadastra cada uma como produto na tabela `products`, na nova categoria
 * `blusa_arrastao`.
 */
...
const TMP = path.resolve(__dirname, "..", "tmp_blusa_arrastao");

const supabase = createClient(SUPABASE_URL, SERVICE_ROLE_KEY);

const ITEMS = Array.from({ length: 6 }, (_, i) => ({
  index: i + 1,
  category: "blusa_arrastao",
  name: "Blusa Arrastão Azul",
}));

async function uploadOne(item) {
  const localPath = path.join(TMP, `b-${String(item.index).padStart(3, "0")}.png`);
  if (!fs.existsSync(localPath)) {
    throw new Error("Imagem nao encontrada: " + localPath);
  }
  const buf = fs.readFileSync(localPath);
  const ts = Date.now();
  const storagePath = `blusa_arrastao/${ts}-blusa-arrastao-azul-${String(item.index).padStart(3, "0")}.png`;
...
(async () => {
  console.log(`Enviando ${ITEMS.length} novas fotos de Blusa Arrastão Azul...\n`);
  let ok = 0;
  let err = 0;
