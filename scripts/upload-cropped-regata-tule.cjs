/**
 * Faz upload de 6 novas fotos de Cropped Regata de Tule Azul Claro para o
 * Supabase Storage e cadastra cada uma como produto na tabela `products`,
 * categoria `regata_tule`.
 */
...
const TMP = path.resolve(__dirname, "..", "tmp_tule");

const supabase = createClient(SUPABASE_URL, SERVICE_ROLE_KEY);

const ITEMS = Array.from({ length: 6 }, (_, i) => ({
  index: i + 1,
  category: "regata_tule",
  name: "Cropped Regata de Tule Azul Claro",
}));
...
async function uploadOne(item) {
  ...
  const ts = Date.now();
  const storagePath = `regata_tule/${ts}-cropped-regata-tule-azul-claro-${String(item.index).padStart(3, "0")}.png`;
...
(async () => {
  console.log(`Enviando ${ITEMS.length} novas fotos de Cropped Regata de Tule Azul Claro...\n`);
  let ok = 0;
  let err = 0;
