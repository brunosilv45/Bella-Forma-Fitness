...
$dst = 'C:\Users\Bruno Carvalho\Documents\BellaFormaFitness\tmp_top_regata'
if (!(Test-Path -LiteralPath $dst)) { New-Item -ItemType Directory -Path $dst | Out-Null }
$files = Get-ChildItem -Path $src | Where-Object {
  $_.Name -like '*2026-05-12_at_11.24.21*' -or
  $_.Name -like '*2026-05-12_at_11.24.22*'
} | Sort-Object Name
$i = 0
foreach ($f in $files) {
  $i++
  $idx = "{0:D3}" -f $i
  $longSrc = '\\?\' + $f.FullName
  $dstPath = Join-Path $dst "t-$idx.png"
