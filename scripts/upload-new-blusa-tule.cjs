/**
 * Faz upload de 5 novas fotos de Blusa Tule Branca para o Supabase
 * Storage e cadastra cada uma como produto na tabela `products`,
 * categoria `blusa_tule` (nova subcategoria do grupo "Blusas").
 */
...
const TMP = path.resolve(__dirname, "..", "tmp_blusa_tule");
...
async function uploadOne(item) {
  ...
  const ts = Date.now();
  const storagePath = `blusa_tule/${ts}-blusa-tule-branca-${String(item.index).padStart(3, "0")}.png`;
...
(async () => {
  console.log(`Enviando ${ITEMS.length} novas fotos de Blusa Tule Branca...\n`);
  let ok = 0;
  let err = 0;
