/**
 * Faz upload de 6 novas fotos de Short Duplo Vermelho para o Supabase
 * Storage e cadastra cada uma como produto na tabela `products`,
 * categoria `short_duplo`.
 */
...
const TMP = path.resolve(__dirname, "..", "tmp_short");

const supabase = createClient(SUPABASE_URL, SERVICE_ROLE_KEY);

const ITEMS = Array.from({ length: 6 }, (_, i) => ({
  index: i + 1,
  category: "short_duplo",
  name: "Short Duplo Vermelho",
}));
...
async function uploadOne(item) {
  ...
  const ts = Date.now();
  const storagePath = `short_duplo/${ts}-short-duplo-vermelho-${String(item.index).padStart(3, "0")}.png`;
...
(async () => {
  console.log(`Enviando ${ITEMS.length} novas fotos de Short Duplo Vermelho...\n`);
  let ok = 0;
  let err = 0;
