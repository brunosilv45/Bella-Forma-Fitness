/**
 * Faz upload de 6 novas fotos de Regata Arrastão Verde para o Supabase
 * Storage e cadastra cada uma como produto na tabela `products`, categoria
 * `regata_arrastao`.
 */
...
const TMP = path.resolve(__dirname, "..", "tmp_regata_verde");
...
const ITEMS = Array.from({ length: 6 }, (_, i) => ({
  ...
  name: "Regata Arrastão Verde",
}));

async function uploadOne(item) {
  const localPath = path.join(TMP, `r-${String(item.index).padStart(3, "0")}.png`);
  if (!fs.existsSync(localPath)) {
    throw new Error("Imagem nao encontrada: " + localPath);
  }
  const buf = fs.readFileSync(localPath);
  const ts = Date.now();
  const storagePath = `regata_arrastao/${ts}-regata-arrastao-verde-${String(item.index).padStart(3, "0")}.png`;
...
(async () => {
  console.log(`Enviando ${ITEMS.length} novas fotos de Regata Arrastão Verde...\n`);
  let ok = 0;
  let err = 0;
