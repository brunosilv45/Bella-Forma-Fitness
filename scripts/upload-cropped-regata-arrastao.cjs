/**
 * Faz upload de 4 novas fotos de Cropped Regata Arrastão Vermelha para o
 * Supabase Storage e cadastra cada uma como produto na tabela `products`,
 * categoria `regata_arrastao`.
 */
...
const TMP = path.resolve(__dirname, "..", "tmp_arrastao");

const supabase = createClient(SUPABASE_URL, SERVICE_ROLE_KEY);

const ITEMS = Array.from({ length: 4 }, (_, i) => ({
  index: i + 1,
  category: "regata_arrastao",
  name: "Cropped Regata Arrastão Vermelha",
}));

async function uploadOne(item) {
  const localPath = path.join(TMP, `a-${String(item.index).padStart(3, "0")}.png`);
  if (!fs.existsSync(localPath)) {
    throw new Error("Imagem nao encontrada: " + localPath);
  }
  const buf = fs.readFileSync(localPath);
  const ts = Date.now();
  const storagePath = `regata_arrastao/${ts}-cropped-regata-arrastao-vermelha-${String(item.index).padStart(3, "0")}.png`;
...
(async () => {
  console.log(`Enviando ${ITEMS.length} novas fotos de Cropped Regata Arrastão Vermelha...\n`);
  let ok = 0;
  let err = 0;
