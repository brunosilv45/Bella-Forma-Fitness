/**
 * Faz upload de 3 novas fotos de Short Saia Rose para o Supabase Storage
 * e cadastra cada uma como produto na tabela `products`, categoria
 * `short_saia`.
 */
...
const TMP = path.resolve(__dirname, "..", "tmp_short_saia");
...
const ITEMS = Array.from({ length: 3 }, (_, i) => ({
  index: i + 1,
  category: "short_saia",
  name: "Short Saia Rosé",
}));

async function uploadOne(item) {
  const localPath = path.join(TMP, `s-${String(item.index).padStart(3, "0")}.png`);
  if (!fs.existsSync(localPath)) {
    throw new Error("Imagem nao encontrada: " + localPath);
  }
  const buf = fs.readFileSync(localPath);
  const ts = Date.now();
  const storagePath = `short_saia/${ts}-short-saia-rose-${String(item.index).padStart(3, "0")}.png`;
...
(async () => {
  console.log(`Enviando ${ITEMS.length} novas fotos de Short Saia Rosé...\n`);
  let ok = 0;
  let err = 0;
